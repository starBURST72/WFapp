...
namespace КР_УБД_V_1._0
{
    public partial class UpdateClientForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DB dB = new DB();
            MySqlCommand command = new MySqlCommand("UPDATE `clients` SET `Route_id` = @Route_id, `Surname` = @Surname, `Lastname` = @lastname, `Birthday` = @birthday, `Name` = @Name, `Address` = @Address, `Phone_number` = @Phone_number WHERE `Clients`.`Client_id` = @ID;", dB.GetConnection());
            ...
            command.Parameters.Add("@ID", MySqlDbType.Int64).Value = textBox7.Text;

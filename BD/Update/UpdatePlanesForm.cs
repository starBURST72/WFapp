...
{
    public partial class UpdatePlanesForm : Form
    {
        public UpdatePlanesForm()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            DB dB = new DB();
            MySqlCommand command = new MySqlCommand("UPDATE `planes` SET `Categories` = @Categories, `AirportFrom` = @AirportFrom, `AirportTo` = @AirportTo, `Seats` = @Seats, `FlyDur` = @FlyDur WHERE `planes`.`Plane_id` = @ID;", dB.GetConnection());
            ...
            command.Parameters.Add("@ID", MySqlDbType.Int64).Value = textBox6.Text;
